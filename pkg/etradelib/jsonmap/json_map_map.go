...
// the key and value parameters by returning new values for either or both.
// The callback may also delete a key by returning an empty string for the new
// key name.
//
...
// function parameters. Either or both may be nil, but note that maps may
// contain slices, which is why JsonMap.Map() allows you to specify mapping
// functions for both maps and slices.
...
func mapMap(
	...
	for mapKey, mapValue := range original {
		...
		// If the new map key is not empty, process it. Otherwise, drop it.
		if newMapKey != "" {
			...
			case JsonSlice:
				...
				// before adding the result to the new map with the new key.
				newMap[newMapKey] = mapSlice(
					newMapValTyped, currentElementPath, ancestorSliceIndex, mapMapFn, sliceMapFn,
				)
			case []interface{}:
			...
			default:
				// The new value is not a map or slice, so just add it to the
				// new map with the new key.
