...
// the value parameter and return a new one.
...
type SliceMapFn func(elementPath []interface{}, ancestorSliceIndex int, index int, value interface{}) interface{}
...
func mapSlice(
	...
	for sliceIndex, sliceVal := range original {
		...
		if sliceMapFn != nil {
			newSliceVal = sliceMapFn(currentElementPath, ancestorSliceIndex, sliceIndex, sliceVal)
		}
		...
		case JsonMap:
			...
			newSlice[sliceIndex] = mapMap(newSliceValTyped, currentElementPath, sliceIndex, mapMapFn, sliceMapFn)
		...
		case JsonSlice:
			...
			// function to it before adding the result to the new slice.
			newSlice[sliceIndex] = mapSlice(newSliceValTyped, currentElementPath, sliceIndex, mapMapFn, sliceMapFn)
		case []interface{}:
